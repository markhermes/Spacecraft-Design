...
for i = 1:numel(time)
    ...
    % update the F matrix;
    F12_update = RotationInertial_fromBodyRates_quat(q_att)*F12;
    F   = [F11,F12_update;F21,F22];
    ...
    if(mod(i,meas_update_period) == 0)
        ...
        q_err       = Quaternion_MULTIPLY(Quaternion_inv(q_att),q_ant(i,:));
        ...
        % debug
        q_att       = Quaternion_MULTIPLY(q_att,q_correct);
...
function R = RotationInertial_fromBodyRates_quat(quat)
